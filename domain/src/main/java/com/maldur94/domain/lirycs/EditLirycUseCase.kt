...
class EditLirycUseCase @Inject constructor(
    @LirycsRepositoryInject private val repository: LirycsRepository,
    @NetworkLayerInject networkLayer: NetworkLayer
) : NetworkLayer by networkLayer, UseCase<Pair<Liryc, Liryc>, StatusResponse?> {

    override suspend fun execute(request: Pair<Liryc, Liryc>): StatusResponse? {
        return call {
            withContext(Dispatchers.IO) {
                repository.editLirycToFirebase(request)
            }
        }
    }
}
